 ...
 *
 * Adding, removing and the owner's posting block all ASK first. Either one is a write the other members
...
export function useGroupInfo(chatId: Id, isGroup: boolean) {
  const { members, isLoading, addMembers, removeMember, setMemberBlocked } = useMembers(
    chatId,
    isGroup,
  )
  ...
  }, [confirm, isConfirming, addMembers, removeMember, setMemberBlocked])
